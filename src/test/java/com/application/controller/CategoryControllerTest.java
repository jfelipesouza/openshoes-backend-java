...
public class CategoryControllerTest {
	...
	@Test
	public void returnsCategoryWhenExistingId() throws Exception {
		ResultActions result= mockMvc.perform(get("/categories/{idcategory}", existingId).accept(MediaType.APPLICATION_JSON));
	result.andExpect(status().isOk());
	}
	
	@Test
	public void returnsStatus404WhenConsultsNonExistingId() throws Exception {
		ResultActions result= mockMvc.perform(get("/categories/{idcategory}", nonExistingId).accept(MediaType.APPLICATION_JSON));
		result.andExpect(status().isNotFound());
	}
	
	@Test
	public void returnCategoryWhenSave() throws Exception {
	...
	public void returnsStatus201WhenUpdateSuccessfully() throws Exception {
		...
		ResultActions result = mockMvc.perform(put("/categories/{idcategory}", existingId)
				.content(jsonBody)
				.contentType(MediaType.APPLICATION_JSON)
				.accept(MediaType.APPLICATION_JSON));
		result.andExpect(status().isOk());
	...
	@Test
	public void returnsStatus404WhenUpdatesNonExistingCategory() throws Exception {
		String jsonBody= objMapper.writeValueAsString(existingCategory);
		ResultActions result = mockMvc.perform(put("/categories/{idcategory}", nonExistingId)
				.content(jsonBody)
				.contentType(MediaType.APPLICATION_JSON)
				.accept(MediaType.APPLICATION_JSON));
		result.andExpect(status().isNotFound());
